 ...
 * @author Gabriel
 */
public class Cmeans_memo implements Clasificador{
    
    private int c;
    private PatronRepresentativo[] centroides;
    
    public Cmeans_memo(int c) {
        if(c>151) this.c=150;
        else this.c = c;
    ...
    private void clasificacion(ArrayList<Patron> patrones) {
        patrones.forEach((e) -> {
            double distC = LeerDatos.calcularDistanciaEuclidiana(e, this.getCentroides()[0]);
            e.setClase(this.getCentroides()[0].getClase());
            for (int i=1; i<this.c;i++){
                double nDistancia = LeerDatos.calcularDistanciaEuclidiana(e, this.getCentroides()[i]);
                if(distC>nDistancia){
                    distC=nDistancia;
